...
def evaluate(loss, input_image, original, noisy_image, output):
    ...
    with tf.compat.v1.Session() as sess:
        ...
        denoise_writer = tf.io.write_file(
        ...
        noisy_image_writer = tf.io.write_file(
...
def main():
    ...
    if not ARGS.train:
        ...
        # get the path name of the image given by the iterator.
...
if __name__ == "__main__":
    ...
    PARSER.add_argument(
        ...
        type=int,
        default=0,
        help="The number of gpus to be used. If set to 0, only the CPU is used.",
    )
